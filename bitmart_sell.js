...
//    create_order_lbank = await lbank.createOrder('cfxt_usdt', 'limit', 'sell', 1, max_buy_lbank_price);
    ...
    if(balance_CFXT_bitmart <= 3000)
    {
        ...
        if(max_buy_bitmart_price >= 0.0023 && max_buy_bitmart_volume >= 3000){
            create_order_bitmart = await bitmart.createOrder('CFXT_USDT', 'limit', 'sell', max_buy_bitmart_volume, max_buy_bitmart_price);
